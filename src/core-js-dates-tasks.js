...
function getNextFriday(date) {
  ...
  const daysUntilFriday = (5 - getDayIndex + 7) % 7;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const countDays = diffInMilliseconds / (1000 * 3600 * 24);
  return Math.abs(Math.round(countDays)) + 1;
}
/**
...
function getNextFridayThe13th(date) {
  ...
  while (iterations < maxIterations) {
    const possibleFridayThe13th = new Date(year, month, 13);
    if (possibleFridayThe13th.getDay() === 5) {
      if (possibleFridayThe13th > date) {
        return possibleFridayThe13th;
      }
    }
    month += 1;
    if (month > 11) {
